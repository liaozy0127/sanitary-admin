      ...
      <div class="items-section">
        ...
        <el-table :data="formData.items" border size="small" style="width: 100%" max-height="400">
          ...
          <el-table-column label="单位" width="80">
            <template #default="{ row }">
              <el-input v-model="row.unit" size="small" :maxlength="50" />
            </template>
          </el-table-column>
          ...
          <el-table-column label="排产方式" width="90">
            <template #default="{ row }">
              <el-input v-model="row.productionType" size="small" :maxlength="50" />
            </template>
          </el-table-column>
...
const onItemMaterialChange = async (id, index) => {
  ...
  // 如果仍未拿到价格，回退到物料默认单价
  if ((!row.platingPrice || Number(row.platingPrice) === 0) && material && material.defaultPrice && Number(material.defaultPrice) > 0) {
...
const queryAndSetPlatingPrice = async (row) => {
  ...
  try {
    ...
    if (data.unitPrice != null && Number(data.unitPrice) > 0) {
      row.platingPrice = Number(data.unitPrice)
    }
  } catch (e) { /* 查不到不影响录入 */ }
}
