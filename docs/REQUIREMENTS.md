...
> 📌 **待开发**：返工单需改造为主从表模式（与收货单/排产单/发货单一致）
...
**业务说明**：每月末生成月度对账汇总，供客户确认。支持从老系统 Excel 导入历史对账数据。
...
- `POST /api/statements/import` — **从老系统 Excel 导入历史对账单**

**对账单 Excel 导入格式**（老系统，122行，前2行表头）：
| 列 | 含义 | 映射 |
|----|------|------|
| A(0) | 产品代码 | material_code |
| B(1) | 产品名称 | material_name |
| C(2) | 工艺要求 | process_name |
| D(3) | 上月结余数量 | 库存初始化用 |
| E(4) | 本月收货（正常）| receipt_qty |
| F(5) | 本月收货合计 | receipt_qty_total |
| G(6) | 本月发货（良品）| shipment_qty |
| H(7) | 本月发货（原件退回）| |
| I(8) | 本月发货合计 | shipment_qty_total |
| J(9) | 本月结余数量 | |
| K(10) | 单价 | unit_price |
| L(11) | 良品金额 | |
...
> ⚠️ **导入幂等性**：按月份+客户+产品代码去重，已存在则 skip

> ⚠️ **库存初始化联动**：对账单导入接口**不**自动初始化库存；库存初始化需单独调用 `POST /api/inventory/init-from-statement` 接口
